...
@section('content')
    <div class="common-area">
        <div class="common-head text-center">
            <h3>Update Embedded celebrational Video</h3>
        </div>
        <div class="row">
            <div class="col-md-6 col-md-offset-3 col-sm-8 col-sm-offset-2">
                ...
                <div class="form-area">
                    <form action="{{url('/videoUpdate/'.$video->id)}}" method="post" enctype="multipart/form-data">
                        {{csrf_field()}}
                        <div class="form-group">
                            <label for="caption">Caption</label>
                            <input type="text" value="{{$video->caption}}" class="form-control" name="caption" id="caption" placeholder="Enter caption">
                            <span class="text-danger">{{$errors->first('caption')}}</span>
                        </div>

                        <div class="form-group">
                            <label for="year">Year</label>
                            <input type="text" value="{{$video->celebration_year}}" name="year" class="form-control some-element" placeholder="Pick date">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="celebration">
                                <option value="{{$video->celebration_name}}">{{$video->celebration_name}}</option>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" value="{{$video->youtube_id}}" name="youtube_id" class="form-control" placeholder="http://www.youtube.com/watch?v= - - - -">
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="descpt" class="form-control" name="descpt" rows="3">{{$video->celebration_descpt}}</textarea>
                            <span class="text-danger">{{$errors->first('descpt')}}</span>
                        </div>



                        <button type="submit" class="btn btn-default">Embedded update</button>
                    </form>
                </div>
            </div>
        </div>
